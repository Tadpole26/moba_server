...
{
	if (!m_oMysql.Init(LOG_LOGIC_INS->m_oConstCfg.m_strMysqlHost
		, LOG_LOGIC_INS->m_oConstCfg.m_strMysqlUser
		, LOG_LOGIC_INS->m_oConstCfg.m_strMysqlPasswd
		, ""
		, LOG_LOGIC_INS->m_oConstCfg.m_uiMysqlPort))
		return false;

	char szBuff[1024] = { 0 };
	sprintf_safe(szBuff, u8"gmws_uiq_rolename_%u", LOG_LOGIC_INS->m_oConstCfg.m_uiGroupId);
	m_strTableName = szBuff;

	sprintf_safe(szBuff, u8"CREATE DATABASE IF NOT EXISTS `%s`", LOG_LOGIC_INS->m_oConstCfg.m_strMysqlAccName.c_str());
	if (-1 == m_oMysql.DBExec(szBuff, eDB_None))
		return false;

	sprintf_safe(szBuff, u8"USE `%s`;", LOG_LOGIC_INS->m_oConstCfg.m_strMysqlAccName.c_str());
	if (-1 == m_oMysql.DBExec(szBuff, eDB_None))
		return false;

	m_oMysql.SetDbName(LOG_LOGIC_INS->m_oConstCfg.m_strMysqlAccName);

	sprintf_safe(szBuff, u8"call sp_create_uiq_rolename_table(%d);"
		, LOG_LOGIC_INS->m_oConstCfg.m_uiGroupId);
...
int CDbInstance::InsertNewRole(int64 llPlayerId, const std::string& strName, bool bTransfer)
{
	...
	uint32 uiArea = LOG_LOGIC_INS->m_oConstCfg.m_uiGroupId;
	if (bTransfer)
	{
